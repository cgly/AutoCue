...
class Attack:
    ...
    def get_transformations(self, current_text, original_text=None, ground_truth_output=None, **kwargs):
        """Applies ``self.transformation`` to ``text``, then filters the list
        of possible transformations through the applicable constraints.
    ...
    def attack(self, example, ground_truth_output):
        ...
        #add prompt label to AttackedText
        example.ground_truth=ground_truth_output
        goal_function_result, _ = self.goal_function.init_attack_example( #查询结果 看是否满足对抗条件
