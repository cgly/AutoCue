...
from textattack.prompt_config import nclasses,class_names
...
class WordSwapMaskedLM(WordSwap):
    ...
    def __init__(
        self,
        method="bae",
        masked_language_model= r"E:\code\textcode\Pro-CLARE\MLM_model\bert-base-uncased",#"bert-base-uncased", #"bert-base-uncased-imdb",
        tokenizer=None,
        max_length=512,
    ...
    def _proattacker_replacement_words(self, current_text, indices_to_modify):
        """Get replacement words for the word we want to replace using Proattacker
        method.
        ...
        for index in indices_to_modify:
            ...
            # Obtain window
            masked_text = masked_text.text_window_around_index(index, self.window_size)+current_text.ground_truth
            masked_texts.append(masked_text)
    ...
    def _get_transformations(self, current_text, indices_to_modify):
        ...
        if self.method == "bert-attack":
        ...
        elif self.method == "bae":#2021年12月23日09:37:01 这个方法中出现了问题 输出词全是real
            ...
            return transformed_texts
        elif self.method == "ProAttacker":
            replacement_words = self._proattacker_replacement_words(
                current_text, indices_to_modify
            )
            transformed_texts = []
            for i in range(len(replacement_words)):
...
class TarWordSwapMaskedLM(WordSwap):
    """Generate potential replacements for a word using a masked language
    model.
    ...
    def __init__(
        ...
        tar_prompt,
        method="bae",
        masked_language_model= r"K:\PLM\bert-base-uncased",#"bert-base-uncased", #"bert-base-uncased-imdb",
        tokenizer=None,
        max_length=512,
        window_size=float("inf"),
        max_candidates=50,
    ...
    def _bae_replacement_words(self, current_text, indices_to_modify):
        ...
        for index in indices_to_modify:
            ...
            # Obtain window
            masked_text = masked_text.text_window_around_index(index, self.window_size)+str(self.tar_prompt)
            masked_texts.append(masked_text)
