...
def evaluate(refs, hypos, use,args):
    ...
    tokenizer = GPT2Tokenizer.from_pretrained(r"E:\code\textcode\Pro-CLARE\MLM_model\gpt2")
    model = GPT2LMHeadModel.from_pretrained(r"E:\code\textcode\Pro-CLARE\MLM_model\gpt2").cuda()
    ...
    #BERT SCORE:
    # with torch.no_grad():
    #     P, R, F1 = score(hypos, refs,model_type=r'E:\code\textcode\Pro-CLARE\MLM_model\roberta-large',num_layers=17, lang='en', verbose=True)
    ...
    return ref_ppl, hy_ppl, 0, sim_score, 0
...
def do_eval(logPath):
    ...
    argparser.add_argument("--USE_cache_path",
                           type=str,
                           default=r'E:\code\textcode\Pro-CLARE\tmp',
    ...
    return ref_ppl, hy_ppl, F1, sim_score, gramar_err
...
if __name__ == '__main__':
    ...
    logPath = r'C:\Users\Rafael\Desktop\LastTar_Texefooler_ag.csv'
    log = pd.read_csv(logPath, header=0, encoding="UTF-8")
    refs = [line.replace('[[', "").replace(']]', "") for line in list(log['original_text'])]
    hypos = [line.replace('[[', "").replace(']]', "") for line in list(log['perturbed_text'])]
